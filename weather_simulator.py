...
for day in range(1, num_days + 1):
    ...
    next_state = np.random.choice(
        states,
        p=[transition_matrix[current_state]["Clear"], transition_matrix[current_state]["Partially cloudy"]]
        if current_state == "Clear"
        else [transition_matrix[current_state]["Clear"], transition_matrix[current_state]["Partially cloudy"]]
    )
